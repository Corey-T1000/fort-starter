...
Build a self-contained HTML viewer that lets the user trace every block of derived content back to its source, with visual cues for what was lifted verbatim, trimmed, edited, or invented.

## When to Use

- **Verifying derivative content** — useful when verifying that two HTML/Markdown sources (e.g., a draft and a production page, a one-pager and the live page it's based on) align block-for-block
- **Source-fidelity audits** — when you need a defensible artifact showing word-for-word source attribution or surfacing where editorial latitude was taken
...
3. **Item display names** — e.g., "Section A", "Section B" (for the per-item card headers)

If user provides just a directory of files, auto-pair by name matching when possible.

## Reference Template

Working template lives at `plugins/fort-tools/skills/diff-view-references/template.html` — a single-file HTML scaffold with:
...
Take the template from `plugins/fort-tools/skills/diff-view-references/template.html` and populate it with the extracted + analyzed content. Each item becomes a `<section class="customer">` — the class name is historical; use it for any item type.
...
- Per-item: block counts on each side (e.g., "Section A: 12 OURS / 44 LIVE blocks paired")
- Aggregate edit-type totals across all items: N VERBATIM / N TRIMMED / N EDITED / N NOVEL
- Path to the output HTML
- Any extraction caveats (e.g., "Source page has no `<article>` wrapper; used deepest H2 common ancestor")
